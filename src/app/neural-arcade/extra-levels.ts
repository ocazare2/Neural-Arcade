    ...
    theory: [
      {
        ...
        level: "Básico",
        body: "Una observación asombrosa del deep learning: la pérdida de un LLM decae como una **power law** suave con compute, parámetros y datos. No hay 'techo' obvio, no hay 'precipicio'. Kaplan et al. (OpenAI, 2020) mostraron que L(N) ≈ (Nc/N)^α con α≈0.076 — pequeños incrementos en parámetros producen mejoras predecibles.\n\nEsto transformó la industria: en lugar de buscar arquitecturas mejores, basta con escalar más. Cada nueva generación de LLM (GPT-3 → GPT-4 → GPT-4o) es básicamente el mismo transformer, escalado 10x en compute. La scaling law te dice de antemano cuánto vas a mejorar.",
      ...
      },
      {
        ...
        level: "Avanzado",
        body: "Hoffmann et al. (DeepMind, 2022) re-entrenaron más de 400 modelos para refutar Kaplan. Descubrieron que el compute-optimal requiere **~20 tokens por parámetro**, no ~1.7 como GPT-3. GPT-3 estaba **undertrained**: si OpenAI hubiera entrenado un modelo más pequeño con más datos, habrían obtenido mejor calidad al mismo costo.\n\nChinchilla 70B con 1.4T tokens usa el mismo compute que GPT-3 175B con 300B tokens, pero supera a GPT-3 en todos los benchmarks. Esta lección redefine cómo se diseñan LLMs: Llama 3 8B se entrena con 15T tokens (1875 tokens/param), muy por encima del ratio Chinchilla. La conclusión: hoy entrenamos modelos 'más pequeños pero más profundamente'.",
        formula: "N_{\\text{opt}} \\propto C^{0.5}, \\quad D_{\\text{opt}} \\propto C^{0.5}",
        formulaExplain: "Ambos, parámetros óptimos y datos óptimos, escalan con la raíz cuadrada del compute total C. Por eso el ratio tokens/parámetro es constante ≈20:1 sin importar la escala. Doblar compute → doblar parámetros Y doblar datos. Kaplan decía N∝C^0.73, D∝C^0.27 (escala N mucho más rápido) — eso llevó a la generación de modelos undertrained.",
      },
      {
  ...
  // ───────────────────────────────────────────────────────
  {
    ...
    theory: [
      ...
      },
      {
        ...
        level: "Experto",
        body: "Medir un LLM es endemoniadamente difícil. Los benchmarks estándar: **MMLU** (57 asignaturas, multiple-choice) mide conocimiento; **HumanEval** (164 problemas Python) mide código; **MT-Bench** (multi-turn, evaluado por GPT-4) mide conversación; **GPQA** (Google-Proof Q&A) mide razonamiento experto. Cada uno tiene sesgos: contamination (el modelo vio el test en entrenamiento), format sensitivity, sampling variance.\n\nLa pila de inference moderna está dominada por **vLLM** con **PagedAttention**: el KV cache se pagina como un SO pagina RAM, eliminando fragmentación externa y permitiendo batching dinámico de secuencias de longitudes distintas. Esto da 2-4x más throughput que HuggingFace transformers. Otra técnica clave es **speculative decoding**: un modelo pequeño (draft) propone K tokens, el grande los verifica en paralelo en un forward pass. Si todos aceptan, generaste K tokens al costo de 1; si algunos rechazan, descartas desde ahí. Aceleración 2-3x sin pérdida de calidad.",
        formula: "\\text{speedup}_{\\text{spec}} = \\frac{K}{\\mathbb{E}[\\text{tokens aceptados}]} \\cdot \\frac{T_{\\text{large}}}{T_{\\text{large}} + T_{\\text{small}}/K}",
        formulaExplain: "Si el draft acierta en promedio α tokens por ronda, generas 1+α tokens al costo de 1 forward del modelo grande + 1 del pequeño. El speedup depende de qué tan bien alineados estén draft y target. Modelos destilados del target dan α cercano a K; modelos genéricos dan α bajo. La técnica brilla en generación larga y determinista (greedy), menos con alta temperatura.",
      },
    ],
  },
];

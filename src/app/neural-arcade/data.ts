...
export const LEVELS: LevelMeta[] = [
  ...
  // ───────────────────────────────────────────────────────
  {
    ...
    icon: "🔤",
    color: "#22d3ee",
    tag: "El alfabeto de la IA",
    summary: "Cómo la IA descompone texto en unidades numéricas procesables.",
    estimatedMin: 12,
    concepts: ["Token", "Vocabulario", "BPE", "Token ID", "Token especial", "Context window"],
    paperRef: { title: "Neural Machine Translation of Rare Words with Subword Units", year: 2015, authors: "Sennrich, Haddow, Birch" },
    glossary: [
      { term: "Token", definition: "Unidad atómica de texto que un modelo procesa. Puede ser una palabra, subpalabra o un solo carácter." },
      { term: "Vocabulario (V)", symbol: "V", definition: "Conjunto finito de todos los tokens que un modelo conoce. GPT-4 tiene ~100k tokens." },
      { term: "BPE", definition: "Byte-Pair Encoding. Algoritmo que aprende a partir de un corpus cuáles pares de bytes/secuencias aparecen más, y los fusiona en un token único." },
      { term: "Token ID", definition: "Entero único asignado a cada token del vocabulario. Internamente el modelo sólo trabaja con IDs, no con texto." },
      { term: "Context window", definition: "Cantidad máxima de tokens que el modelo puede procesar a la vez. GPT-4 Turbo: 128k, Claude 3: 200k." },
    ],
    theory: [
      {
        title: "¿Qué es un token realmente?",
        level: "Básico",
        body: "Un LLM no lee texto, lee números. El paso inicial es partir tu frase en piezas llamadas **tokens**, y asignar a cada una un ID entero. Una palabra común puede ser un solo token; una palabra rara se parte en subpalabras. Esto permite que el modelo maneje lenguas, jerga y errores tipográficos sin tener un vocabulario infinito.\n\nEjemplo: 'inteligencia' → ['intel', 'igencia']. 'gato' → ['gato']. El número de tokens determina el costo y la velocidad de inferencia.",
      },
      {
        title: "Byte-Pair Encoding (BPE) — el algoritmo estándar",
        level: "Intermedio",
        body: "BPE construye el vocabulario fusionando iterativamente los pares más frecuentes. Empieza con caracteres y sube de nivel.\n\n**Algoritmo:**\n1. Inicializa el vocabulario con todos los bytes (256 entradas).\n2. Cuenta cada par adyacente de tokens en el corpus.\n3. Fusiona el par más frecuente en un nuevo token.\n4. Repite hasta alcanzar el tamaño de vocabulario objetivo V.",
        formula: "\\text{score}(a,b) = \\frac{\\text{freq}(a,b)}{\\text{freq}(a) \\cdot \\text{freq}(b)}",
        formulaExplain: "En variantes modernas como WordPiece (BERT) se usa esta fórmula normalizada: favorece fusiones que son frecuentes en relación a cuán comunes son sus partes, no sólo las absolutamente más frecuentes.",
      },
      {
        title: "Tokens especiales y contexto",
        level: "Avanzado",
        body: "Todo LLM reserva tokens especiales que NUNCA aparecen en texto natural:\n• **<BOS>** — Beginning Of Sequence\n• **<EOS>** — End Of Sequence (señal para detener generación)\n• **<PAD>** — Padding (rellena secuencias más cortas en batch)\n• **<UNK>** — Unknown (token fuera de vocabulario, raro en BPE)\n\nLa **context window** es el máximo de tokens que el modelo atiende simultáneamente. Excederla implica truncar el contexto más antiguo (sliding window).",
      },
      {
        title: "Tokenización a nivel de bytes — el truco de GPT-2",
        level: "Experto",
        body: "GPT-2 introdujo tokenización a **byte-level** en lugar de carácter-level. Cada byte (0-255) se mapea a un símbolo Unicode visible. Esto garantiza que cualquier cadena de bytes (incluyendo emojis, JSON corrupto, binarios) pueda tokenizarse sin recurrir a <UNK>.\n\nLa desventaja: secuencias largas de bytes poco frecuentes (ej. texto en tailandés para un modelo entrenado mayoritariamente en inglés) generan muchos tokens por palabra, multiplicando el costo.",
      },
    ],
  },
